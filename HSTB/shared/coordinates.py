...
from .MyExceptions import FormattingException
...
def GetCoordFromUser(text_lat="Enter Latitude", text_lon="Enter Longitude", title_lat="Latitude", title_lon="Longitude", def_lat='', def_lon='',
                     parent=None, assume_W=False, assume_S=False):
    import wx
    latstr = "fail"
    lat = None
    formats = "\n".join([constructor.supported_formats for constructor in (DecDegreesCoordinate, DMCoordinate, DMSCoordinate)])
    text_lat = text_lat + '\n\n' + formats
    while latstr and lat is None:  # bail out when we get a legit lat value or user cancels (returns "" for latstr)
        latstr = wx.GetTextFromUser(("" if latstr == "fail" else "Retry -- use formats below\n\n") + text_lat, title_lat,
                                    (def_lat if latstr == "fail" else latstr), parent=None)
    ...
    if latstr:  # user didn't cancel
        ...
        while lonstr and lon == None:  # bail out when we get a legit lat value or user cancels (returns "" for latstr)
            lonstr = wx.GetTextFromUser(("" if lonstr == "fail" else "Retry -- use formats below\n\n") + text_lon, title_lon,
                                        (def_lon if lonstr == "fail" else lonstr), parent=None)
...
class BaseCoordinate:
    ...
    def __init__(self, lat, lon):
        if isinstance(lat, (int, float)) and isinstance(lon, (
        int, float)):  # pass non-string value straight through; also, avoid replacing 'e' in case of (lon) scientific notation formatting
    ...
    def DMS_Caris(self):
        latd, latm, lats = BaseCoordinate._dms(self.lat)
        if self.lat < 0: latd *= -1
        lond, lonm, lons = BaseCoordinate._dms(self.lon)
        if self.lon < 0: lond *= -1
        newlats = round(lats, 2)
        newlons = round(lons, 2)
        LatStr = ('% 03d:%02d:%02d' % (latd, latm, newlats)).strip()  # remove leading space from positive values
        LonStr = ('% 04d:%02d:%02d' % (lond, lonm, newlons)).strip()  # remove leading space from positive values
